...
public class LITPNetworkTestRunner extends TorTestCaseHelper {

    Logger logger = Logger.getLogger(LITPNetworkTestRunner.class);
    ...
    @TestId(id = "CXP9030910-1", title = "Upgrade ERIClitpnetwork rpm on the MS")
    @Test(groups={"ACCEPTANCE"})
    public void upgradeERIClitpnetworkRPM() throws TimeoutException, FileNotFoundException {

        rpmUpgradeOperator.initialise("network");
     ...
     * @PRIORITY HIGH
     */
    @TestId(id = "CXP9030910-2", title = "Run python test cases for ERIClitpnetwork package")
    @Test(groups={"CDB_REGRESSION", "ACCEPTANCE"})
    public void runERIClitpnetworkTests() {
     ...
     * @PRE Execution of test {@link #runERIClitpnetworkTests()}
     * @PRIORITY HIGH
     * @param className
     * @param name
     * @param failures
     */
    @TestId(id = "CXP9030910-3", title = "Parse xml outputs from python tests")
